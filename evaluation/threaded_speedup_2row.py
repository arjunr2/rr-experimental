...
Top panel:    X = devices,    dodge = benchmarks.
Bottom panel: X = benchmarks, dodge = devices.
...
def style_ax(ax, xtick_labels, ylabel=None, tick_fontsize=12, x_positions=None):
...
def main():
    ...
    style_ax(ax_l, dlabels, ylabel="Threaded Speedup (×)", x_positions=xl, tick_fontsize=13)
    ax_l.legend(handles=legend_handles, fontsize=12, framealpha=0.9,
                loc="upper left", ncol=2, handletextpad=0.4, labelspacing=0.2)
    ...
    style_ax(ax_r, blabels_r, ylabel="Threaded Speedup (×)", tick_fontsize=13, x_positions=xr)
    ax_r.legend(handles=legend_handles_r, fontsize=12, framealpha=0.9, ncol=2,
                loc="upper left", handletextpad=0.4)

    PLOTS_DIR.mkdir(parents=True, exist_ok=True)
    out = PLOTS_DIR / "threaded_speedup_2row.pdf"
