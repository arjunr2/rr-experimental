...
Top panel:    X = devices,    dodge = benchmarks, 3 shaped dots per strip.
Bottom panel: X = benchmarks, dodge = devices,    3 shaped dots per strip.
Shape encodes replay variant: circle=1(wasmtime-native), diamond=2(wasmtime-wasm), square=3(wizeng).
Color encodes benchmark (top) or device (bottom).
Y axis scale shared; both panels show Y tick labels."""
...
def style_ax(ax, xtick_labels, ylabel=None, tick_fontsize=11, x_positions=None):
...
def main():
    ...
    style_ax(ax_l, dlabels, ylabel="Recording + Streaming Slowdown (x)", x_positions=xl, tick_fontsize=13)
    ax_l.legend(fontsize=13, framealpha=0.9, loc="upper right", ncol=2,
    ...
    style_ax(ax_r, blabels_r, ylabel="Recording + Streaming Slowdown (x)",
             tick_fontsize=13, x_positions=xr)
    device_legend = ax_r.legend(fontsize=13, framealpha=0.9, loc="upper right",
    ...
    out = PLOTS_DIR / "record_streaming_overhead_2row.pdf"
